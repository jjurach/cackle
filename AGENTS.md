...
2.  **Is it a Trivial Change?** → Make the change, then create a **Simplified Change Doc**.
...
- Present the full plan to the developer.
...
- **`dev_notes/`**: All AI-generated plans and change logs.

---

## 3. Documentation Structures (Checklists)

**MANDATORY:** All plans and change docs you create must include these sections. Full, detailed templates are in `docs/templates.md`.
...
4.  **File Naming is Mandatory:** All plans and change logs in `dev_notes/` MUST use the `YYYY-MM-DD_HH-MM-SS_description.md` format.
5.  **Slack notification is Mandatory:** Notify using the slack-notifications mcp service each time you commit to local git repo.
